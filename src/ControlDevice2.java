...
public class ControlDevice2 {
    private static LogFM logFM=LogFM.getInstance(ControlDevice2.class);
    public static JSONObject queryDoor(){
        return queryDoorWithWifi();
    }
    public static JSONObject queryDoorWithRF24(){
        ...
        return parseQueryDoorStateData(receive);
    }
    ...
    public static JSONObject waitLockAfterDoorOpenAndCloseWithWifi(){
         ...
         *
         * */
    public static JSONObject waitLockAfterDoorOpenAndCloseWithRF24(){
        ConnectDeviceBase connectDeviceBase= RF24.getInstance();
        String receive=connectDeviceBase.connect(ConnectDeviceBase.DoorLockDevice,0l);
        boolean doorstate=parseDoorState(receive);
        if(doorstate){
            for (int i = 0; i < 60; i++) {
                try {
                    Thread.sleep(1000);
                ...
                String temp=connectDeviceBase.connect(ConnectDeviceBase.DoorLockDevice,0l);
                boolean doorstatetemp=parseDoorState(temp);
                if(!doorstatetemp){
                    break;
                }
            }
    ...
    public static JSONObject lockDoorWithRF24(){
        logFM.p();
        ...
        for (int i = 0; i < 40; i++) {
                ...
                if ((!receive.contains("err"))&&!receive.equals("0")&&parseDoorLockState(receive)) {
                    logFM.d("锁门耗时:"+(System.currentTimeMillis()-time));
                    parseQueryDoorStateData(receive);
                    devicecheck.setStopFlag(false);
                    try {
